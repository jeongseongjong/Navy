...
//				proDao.existing_size_delete(s_code);
	...
	public int existing_update(ProductVO productVO, 
		...
		int qty = 0;
		log.debug("기존 상품 VO 정보 : "+productVO);
		// 기존 DB에 저장된 수량
		List<InventoryChangeVO> exQtyList = new ArrayList<>();
		InventoryChangeVO icVO ;
		
		for(int i = 0 ; i < existing_color.length ; i ++) {
			// 변동량 계산 ( 현재 수량 - 기존 수량 )
			icVO = new InventoryChangeVO();
			icVO.setCh_p_code(productVO.getP_code());
			icVO.setCh_p_name(productVO.getP_name());
			icVO.setCh_price(productVO.getP_price());
			icVO.setCh_date(DateTime.Date());
			icVO.setCh_time(DateTime.Time());
			icVO.setCh_qty(existing_qty[i] - ex_qty[i]);
			if(icVO.getCh_qty() > 0) {
				icVO.setCh_inout(1);
			} else {
				icVO.setCh_inout(0);
...
//			proUpdateVO.setS_size(existing_size[i]);
		...
		proDao.update_changes(exQtyList);
		...
		if(images[0] != null) {
			...
			for (MultipartFile f : fileList) {
				...
				log.debug("파일 리스트 여러개 : " + f.getOriginalFilename());
				proImgVO = new ProductImgVO();
				proImgVO.setP_img_p_code(productVO.getP_code());
				proImgVO.setP_img_origin_name(f.getOriginalFilename());
