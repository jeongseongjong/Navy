	...
	<c:otherwise>
		...
		<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
			<c:forEach items="${CART_LIST}" var="cart">
				<div class="cart_list" data-id="${cart.bk_id}">
					<div class="col-9">
						...
						<p>
							<label>수량</label> <input type="number" name="p_qty"
								...
								type="hidden" name="seq" value="${cart.bk_id}">
							<button class="btn_one_qty" data-id="${cart.bk_id}">수량변경</button>
							<button class="btn_one_delete" data-id="${cart.bk_id}">&times;</button>
							<input type="checkbox" class="cart_list_check"
								value="${cart.bk_id}">
						</p>
					</div>
					<p>
